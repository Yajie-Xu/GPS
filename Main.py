...
def main(args):
    ...
    candidates = extract_good_candidates_by_LQ(candidates, LQ_thres=0.3, num_of_generation=30000)
    method = to_method_object('TF_IDF')
    method.train(contexts_train, responses_train)
    print(f"Test set size: {len(test_x_text)}")
    ...
    print(f"Size of good_candidates_index: {len(good_candidates_index)}")
    good_candidates = [[candidates[y] for y in x] for x in good_candidates_index]
 
    for x in good_candidates_index:
      print("------SIZE:",len(x))
 
    ''' Module 3: Response Selection '''
    METHODS = ['TF_IDF', 'BM25', 'USE_SIM', 'USE_MAP', 'USE_LARGE_SIM', 'USE_LARGE_MAP', 'ELMO_SIM', 'ELMO_MAP', 'BERT_SMALL_SIM', 'BERT_SMALL_MAP', 'BERT_LARGE_SIM', 'BERT_LARGE_MAP', 'USE_QA_SIM', 'USE_QA_MAP']
    for method_name in METHODS[0:3]:
        print(method_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='reddit', choices=['reddit', 'gab', 'conan',"sample"])
